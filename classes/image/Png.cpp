...
unsigned char* MPng::LoadPNG(const char* inFileName)
{
	...
	if(!pFile || err)
	{
		LogFile<<"Can not open png file"<<std::endl;
		return NULL;
	}
	...
	size_t len;
	err = _dupenv_s(&p, &len, "SCREEN_GAMMA");
	if(p && !err) display_exponent = atof(p);
	...
	if(rc != 0)
	{
		...
		{
			case 1:
				LogFile<<"This is not a PNG file: incorrect signature"<< std::endl;
				return NULL;
			case 2:
				LogFile<<"There is bad IHDR (libpng longjmp)"<< std::endl;
				return NULL;
			case 4:
				LogFile<<"Insufficient memory"<< std::endl;
				return NULL;
			default:
				LogFile<<"Unknown readpng_init() error"<< std::endl;
				return NULL;
		}
	}
	if(ReadPngGetBgcolor(&bg_red, &bg_green, &bg_blue) > 1)
	{
		...
		LogFile<<"ReadPngGetBgcolor error"<< std::endl;
		return NULL;
	}
	...
	if(!pImageData)
	{
		LogFile<<"Png no data readed"<< std::endl;
		return NULL;
	}
